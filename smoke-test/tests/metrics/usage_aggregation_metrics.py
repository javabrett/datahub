...
# Micrometer dotted names export to Prometheus with underscores (e.g. datahub_usage_input_bytes_total).
...
# Mirrors UsageActorClassResolver.KNOWN_SUPPORT_ACTOR_URNS (Constants.DATAHUB_ACTOR, ADMIN_ACTOR).
BUILTIN_SUPPORT_USER_URNS = (
    "urn:li:corpuser:datahub",
    "urn:li:corpuser:admin",
)
DATAHUB_SUPPORT_USER_URN = BUILTIN_SUPPORT_USER_URNS[0]
ADMIN_SUPPORT_USER_URN = BUILTIN_SUPPORT_USER_URNS[1]
...
def is_builtin_support_user_urn(urn: str) -> bool:
    return urn in BUILTIN_SUPPORT_USER_URNS


def expected_actor_class_for_support_session(auth_session) -> str:
    """Map the logged-in support corp user to usage aggregation actor_class."""
    urn = session_corp_user_urn(auth_session)
    if is_builtin_support_user_urn(urn):
        return "support"
    return "regular"
        ...
        result = execute_graphql(
            ...
            """,
            {
                "input": {
                    ...
                    "actorUrn": actor_urn,
                    "duration": "ONE_HOUR",
                    "name": "usage-aggregation-support-probe",
                }
            },
            expect_errors=True,
        )
...
def expected_actor_class_for_admin_session(auth_session) -> str:
    """Deprecated alias — support sessions use actor_class=support."""
    return expected_actor_class_for_support_session(auth_session)


def graphql_metadata_query_tags(actor_class: str) -> Dict[str, str]:
    return {
        "usage_operation": "metadata_query",
        "request_api": "graphql",
