...
| ------------------------------------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------- | ------- |
| `USAGE_AGGREGATION_ENABLED`                            | `false` (`true` in Docker quickstart/debug compose) | Enable in-memory API usage aggregation (OSS Micrometer path; not billing) | GMS     |
| `USAGE_AGGREGATION_MICROMETER_EXPORT_ENABLED`          | `true`                                              | Export aggregation to Micrometer on flush                                 | GMS     |
| `USAGE_AGGREGATION_MAX_WINDOW_SECONDS`                 | `300`                                               | Max in-memory window before flush                                         | GMS     |
| `USAGE_AGGREGATION_MAX_CARDINALITY`                    | `10000`                                             | Cardinality threshold for early flush                                     | GMS     |
| `USAGE_AGGREGATION_FLUSH_INTERVAL_SECONDS`             | `60`                                                | Scheduled flush interval                                                  | GMS     |
| `USAGE_AGGREGATION_FLUSH_RETRY_ATTEMPTS`               | `3`                                                 | Publish retry attempts before re-merge on failure                         | GMS     |
| `USAGE_AGGREGATION_FLUSH_RETRY_INITIAL_BACKOFF_MILLIS` | `100`                                               | Initial backoff between flush publish retries (ms)                        | GMS     |

Aggregation is **GMS-only**. MCE, MAE, and upgrade services force `datahub.usage.aggregation.enabled=false`.
