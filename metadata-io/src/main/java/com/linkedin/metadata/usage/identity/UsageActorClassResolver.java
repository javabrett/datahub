...
public class UsageActorClassResolver {
  ...
  private static final Set<String> KNOWN_SUPPORT_ACTOR_URNS =
      Set.of(Constants.DATAHUB_ACTOR, Constants.ADMIN_ACTOR);
  ...
  private UsageActorClass resolveUncached(
    ...
    if (isKnownSupportActor(usageIdentity) || isKnownSupportActor(actorUrn)) {
    ...
    if (usageIdentity != null && usageIdentity.startsWith(Constants.URN_LI_PREFIX + "corpuser:")) {
      ...
      CorpUserFlags flags = requestCorpUserFlags(opContext, usageIdentity);
      if (flags.system()) {
        return UsageActorClass.SYSTEM;
      }
  ...
  private static boolean isKnownSupportActor(@Nullable String actorUrn) {
    return actorUrn != null && KNOWN_SUPPORT_ACTOR_URNS.contains(actorUrn);
  }
